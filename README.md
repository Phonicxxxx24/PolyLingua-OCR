...
Uploads an image or PDF and runs full OCR pipeline.

Response includes:

- job_id
- paragraphs with text, translation, lang, conf, and geometry
- annotated_image URL
- download_json URL
- download_pdf URL
...
Downloads JSON output for a processed job.

### GET /download/pdf/<job_id>

Downloads translated PDF for a processed job.

### POST /back_translate

Back translates English text to selected language.

Request JSON:

```json
{
  "text": "Welcome to the multilingual OCR system",
  "target_lang": "ar"
}
```

Supported target_lang values:
...
- Poppler is required only for PDF input.
- OCR quality depends on source image quality and language packs.
- Back translation depends on external translation provider availability.

## Troubleshooting

- If OCR fails, verify Tesseract path and installed language packs.
- If PDF conversion fails, verify Poppler path in config.py.
- If app does not start, ensure dependencies are installed in active virtual environment.

## License

Use and modify this project as needed for learning and hackathon development.
